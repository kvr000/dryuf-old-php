...
class ClassMetaJava extends \net\dryuf\core\Object implements \net\dryuf\app\ClassMeta
{
	...
	*/
	public static function		openCached($appContainer, $dataClass, $dataView)
	{
		...
		$meta = \net\dryuf\core\Dryuf::getVmCached(__CLASS__, $dataClass, $dataView);
		if (is_null($meta)) {
			$meta = new \net\dryuf\app\ClassMetaJava($dataClass, $dataView);
			$meta->parseDataDescription();
			\net\dryuf\core\Dryuf::putVmCached(__CLASS__, $dataClass, $dataView, $meta);
		}
		return $meta;
	}
	...
	*/
	...
	{
		if (!is_null($getter = $fieldDef->getGetter())) {
			return $o->$getter();
		}
		else {
			$name = $fieldDef->name();
	...
	*/
	...
	{
		if (!is_null($setter = $fieldDef->getSetter())) {
	...
	*/
	protected function		parseDataDescription()
	{
		...
		$fieldDefsList = new \net\dryuf\util\LinkedList();
		$refClass = new \ReflectionClass(\net\dryuf\core\Dryuf::convertClassname($this->dataClass));
		...
		foreach ($this->fieldOrder as $fieldName) {
			...
			if ($refClass->hasProperty($fieldName))
				$fieldDef->setField($fieldName);
			if ($refClass->hasMethod("get".ucfirst($fieldName)))
				$fieldDef->setGetter("get".ucfirst($fieldName));
			if (!is_null($fieldDef->getField())) {
				$fieldDef->setType(\net\dryuf\core\Dryuf::getFieldType($this->dataClass, $fieldDef->getField()));
			}
			elseif (!is_null($fieldDef->getGetter())) {
				$fieldDef->setType(\net\dryuf\core\Dryuf::getMethodReturnType($this->dataClass, $fieldDef->getGetter()));
			}
			else {
				throw new \net\dryuf\core\RuntimeException("no field nor getter defined for ".$this->dataClass.".".$fieldName);
			}
			if ($refClass->hasMethod("set".ucfirst($fieldName)))
				$fieldDef->setSetter("set".ucfirst($fieldName));
	...
	*/
	...
	{
		if (!is_null($fieldDef->getField()) && !is_null(($a = \net\dryuf\core\Dryuf::getFieldAnnotation($this->dataClass, $fieldDef->getField(), $annotation))))
			return $a;
		if (!is_null($fieldDef->getGetter()) && !is_null(($a = \net\dryuf\core\Dryuf::getFieldAnnotation($this->dataClass, $fieldDef->getGetter(), $annotation))))
			return $a;
		if (!is_null($fieldDef->getSetter()) && !is_null(($a = \net\dryuf\core\Dryuf::getFieldAnnotation($this->dataClass, $fieldDef->getSetter(), $annotation))))
			return $a;
		return null;
	}
