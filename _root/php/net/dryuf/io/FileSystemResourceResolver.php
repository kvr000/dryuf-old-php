	...
	public function			getResource($path)
	{
		foreach ($this->paths as $pathDef) {
			...
			if (file_exists($file)) {
				...
				$fileData = \net\dryuf\io\FileDataImpl::createFromFile($file);
	...
	*/
	public function			getResourcePaths($path)
	{
		...
		foreach ($this->paths as $pathDef) {
			...
			if (!is_dir($file))
				continue;
			foreach (\net\dryuf\io\DirUtil::filterEntries(function($entry) use ($file) { $full = $file.$entry; return is_dir($full) ? $entry."/" : $entry; }, $file) as $name) {
				$result->add($name);
			}
		}
		return $result;
