...
abstract class AbstractObjectOperController extends \net\dryuf\core\Object implements \net\dryuf\oper\ObjectOperController, \net\dryuf\core\AppContainerAware
{
	...
	*/
	public function			findActioner($actionName)
	{
		$foundMethod = null;
		$foundOperRules = null;
		foreach (\net\dryuf\core\Dryuf::listMethodsByAnnotation(get_class($this), 'net\dryuf\oper\ObjectOperRules') as $method => $operRules) {
			if ($operRules->value() === $actionName) {
		...
		return new \net\dryuf\oper\AbstractObjectOperController\ActionerImpl(
			$actionName,
			$foundOperRules,
			function ($operContext, $ownerHolder) use ($foundMethod, $foundOperRules) {
				if ($foundOperRules->actionClass() != 'void') {
					return $this->$foundMethod($operContext, $ownerHolder, $operContext->getActionData($foundOperRules->actionClass()));
				}
				else {
					return $this->$foundMethod($operContext, $ownerHolder);
				}
			}
		);
	}
	...
	*/
	public function			operateStaticCommon($operContext, $ownerHolder)
	{
		...
		case \net\dryuf\oper\ObjectOperMethod::LISTING:
