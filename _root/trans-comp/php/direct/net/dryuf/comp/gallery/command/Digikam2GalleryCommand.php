...
class Digikam2GalleryCommand extends \net\dryuf\process\command\AbstractCommand
{
	...
	*/
	...
	{
		(=f_I_x=)System.out.format("%-19s %-39s %-39s %8s\n", "label", "specificPath", "relativePath", "id")(=x_I_f=);
		foreach ($this->galleryReader->listGalleries() as $gallery) {
			(=f_I_x=)System.out.format("%-19s %-39s %-39s %8d\n", gallery.get("label"), gallery.get("specificPath"), gallery.get("relativePath"), gallery.get("id"))(=x_I_f=);
		}
		return 0;
	}
	...
	*/
	protected function		processGenerating()
	{
		if ($this->galleryReader->setGalleries($this->options->get("p"), $this->options->get("g")) == 0) {
			(=f_I_x=)System.err.format("Warning: no gallery of name %s found\n", (String)options.get("g"))(=x_I_f=);
		}
