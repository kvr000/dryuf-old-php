...
abstract class ConversionUtil extends \net\dryuf\core\Object
{
	...
	*/
	...
	{
		if ((=f_I_x=)Number.class.isAssignableFrom(clazz)(=x_I_f=)) {
			return true;
		}
		elseif ((=f_I_x=)clazz.isPrimitive()(=x_I_f=)) {
			return true;
		}
		elseif ((=f_I_x=)Boolean.class.isAssignableFrom(clazz)(=x_I_f=)) {
			return true;
		}
		elseif ((=f_I_x=)String.class.isAssignableFrom(clazz)(=x_I_f=)) {
			return true;
		}
		elseif ((=f_I_x=)Character.class.isAssignableFrom(clazz)(=x_I_f=)) {
			return true;
		}
		else {
			return false;
	...
	*/
	public static function		convertToClass($target, $source)
	{
		...
		elseif (get_class($source) == $target) {
			return $source;
		}
		elseif (((=f_I_x=)Number.class.isAssignableFrom(target)(=x_I_f=) || (=f_I_x=)target.isPrimitive()(=x_I_f=)) && is_numeric($source)) {
			$number = $source;
			if ($target == 'integer' || $target == 'int')
				return intval(intval($number));
			if ($target == 'integer' || $target == 'long')
				return intval(intval($number));
			if ($target == 'double' || $target == 'float')
				return floatval(floatval($number));
			if ($target == 'double' || $target == 'double')
				return doubleval(floatval($number));
			if ($target == 'integer' || $target == 'short')
				return (=f_I_x=)Short.valueOf(number.shortValue())(=x_I_f=);
			if ($target == 'integer' || $target == 'byte')
				return (=f_I_x=)Byte.valueOf(number.byteValue())(=x_I_f=);
			if ($target == 'boolean')
				return \net\dryuf\core\Dryuf::convertBool(floatval($number) != 0);
			throw new \net\dryuf\core\RuntimeException("cannot convert ".get_class($number)." to ".$target);
		}
		elseif ((=f_I_x=)String.class.isAssignableFrom(target)(=x_I_f=)) {
			return $source;
		}
		elseif ((=f_I_x=)Boolean.class.isAssignableFrom(target)(=x_I_f=) || $target == 'boolean') {
			...
			foreach ($map->entrySet() as $entry) {
				$field = \net\dryuf\core\Dryuf::getClassField(get_class($result), $entry->getKey());
				\net\dryuf\core\Dryuf::setFieldValue($result, $field, \net\dryuf\core\ConversionUtil::convertToClass((=f_I_x=)field.getType()(=x_I_f=), $entry->getValue()));
			}
			return $result;
		}
	}
	...
	*/
	public static function		parseStringToClass($target, $source)
	{
		...
		elseif (!is_null(($wrapperClass = self::$primitiveMap->get($target)))) {
			return \net\dryuf\core\Dryuf::invokeMethod(null, \net\dryuf\core\Dryuf::getClassMethod($wrapperClass, "valueOf", 'string'), $source);
		}
		else {
			return \net\dryuf\core\Dryuf::invokeMethod(null, \net\dryuf\core\Dryuf::getClassMethod($target, "valueOf", 'string'), $source);
		}
	}
