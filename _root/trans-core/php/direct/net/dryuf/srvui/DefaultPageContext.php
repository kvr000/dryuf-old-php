...
class DefaultPageContext extends \net\dryuf\core\Object implements \net\dryuf\srvui\PageContext
{
	...
	*/
	public function			getPathElement()
	{
		if (is_null($this->remainPath))
			return null;
		$pathMatch = self::$pathMatcher->matcher($this->remainPath);
		if ($pathMatch->matches()) {
			$parsed = $pathMatch->group(1);
			try {
				$this->processingPath = \java\net\URLDecoder::decode($pathMatch->group(1), "UTF-8");
			}
			...
			if (is_null(($this->remainPath = $pathMatch->group(3)))) {
			}
			else {
				$this->currentPath .= "/";
				$this->realPath .= "/";
	...
	*/
	...
	{
		$lastMatch = self::$lastMatcher->matcher($this->currentPath);
		...
		$this->realPath = strval(substr($this->realPath, 0, strlen($this->realPath)-strlen($lastMatch->group(2))));
		$this->remainPath = !is_null($this->remainPath) ? $lastMatch->group(2).$this->remainPath : $lastMatch->group(2);
	}
	...
	*/
	...
	{
		$lastMatchWithoutSlash = self::$lastMatcherWithoutSlash->matcher($this->currentPath);
	...
	*/
	...
	/**
	@\net\dryuf\core\Type(type = 'java\util\regex\Pattern')
	*/
	static				$pathMatcher;
	...
	*/
	...
	@\net\dryuf\core\Type(type = 'java\util\regex\Pattern')
	*/
	static				$lastMatcherWithoutSlash;

	public static function		_initManualStatic()
	{
		self::$pathMatcher = \java\util\regex\Pattern::compile("^([^/]+)(/+(.*))?\$");
		self::$lastMatcher = \java\util\regex\Pattern::compile("^(|.*/)([^/]+/*)\$");
		self::$lastMatcherWithoutSlash = \java\util\regex\Pattern::compile("^(|.*/)([^/]+)/*\$");
	}
